			...
			$.fn
					.extend( {
						waterfall : function(opt) {
							...
							render(
									$(this).find(setting.cell_selector)
											.detach(), false); //重排已存在元素时强制不渐显
							waterfall._scrollTimer2 = null;
							$(window).bind(
									'scroll',
									function() {
										clearTimeout(waterfall._scrollTimer2);
										waterfall._scrollTimer2 = setTimeout(
												onScroll, 300);
									});
							...
							$(window).bind(
									...
									function() {
										...
										waterfall._scrollTimer3 = setTimeout(
												onResize, 300);
									});
						}
					});
				...
				.each(function(i) {
					if (!setting.auto_imgHeight || setting.insert_type == 2) {//如果给出了图片高度，或者是按顺序插入，则不必等图片加载完就能计算列的高度了
							if (setting.insert_type == 1) {
								...
								insert2($(elements).eq(i), i, setting.fadein
										&& fadein);//插入元素   
							}
						...
						if ($(this)[0].nodeName.toLowerCase() == 'img'
							...
							var src = $(this)[0].nodeName.toLowerCase() == 'img' ? $(
									this).attr('src')
									: $(this).find(setting.img_selector).attr(
											'src');
